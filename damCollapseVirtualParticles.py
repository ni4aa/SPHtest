...
def main():
    np.random.seed(0)
    positions = set_positions(PARTICLES)
    ...
    for iter in tqdm(range(N_TIME_STEPS)):
        ...
        if temp_botton:
            ...
            full_velocities = np.concatenate((velocities, new_bottom_velocities), axis=0)
        else:
        ...
        forces = calculate_forces(neighbor_ids, distances, full_positions, pressures, full_velocities, densities)

        # Euler Step
        velocities = velocities + TIME_STEP_LENGTH * forces[:len(velocities), :] / densities[:len(velocities), np.newaxis]
        ...
        tramp = ((positions[:, 0] < 650) & (positions[:, 0] > 550)) & (positions[:, 1] < 0.4 * (positions[:, 0]-550))

        positions[tramp, 1] = (positions[tramp, 0] - 550) * 0.4
        abs_velocities = np.array([np.linalg.norm(velocity) for velocity in velocities[tramp, :]])
        velocities[tramp, 0] = -DAMPING_COEFFICIENT * 0.4 * abs_velocities
        velocities[tramp, 1] = -DAMPING_COEFFICIENT * 0.6 * abs_velocities
