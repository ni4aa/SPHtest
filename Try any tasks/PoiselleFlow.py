...
def calculate_acceleration(neighbor_ids, distances, positions, pressures, velocities, densities):
    ...
    for i in range(len(neighbor_ids)):
        for j_in_list, j in enumerate(neighbor_ids[i]):
            ...
            accelerations[i] += DYNAMIC_VISCOSITY * PARTICLE_MASS * (velocities[i] - velocities[j])/distances[i][j_in_list] * pressures[i] /\
...
if __name__ == '__main__':
    ...
    for iter in tqdm(range(N_TIME_STEPS)):
        ...
        if iter <= 300:
            out_of_right_boundary = positions[:, 0] > 300
        ...
        if 250 < 350:
            CONSTANT_FORCE = np.array([0.001, 0])
        else:
            CONSTANT_FORCE = np.array([0, 0])
        ...
        if iter % PLOT_EVERY == 0:
            ...
            # plt.xlim(20 + min(positions[:, 0]),20 + max(positions[:,0]))
            plt.xticks([], [])
            plt.yticks([], [])
            plt.tight_layout()
            plt.draw()
